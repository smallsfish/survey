...
<form class="layui-form" id="provinceForm"> <!-- 提示：如果你不想用form，你可以换成div等任何一个普通元素 -->
    ...
    <div class="layui-form-item" style="margin-top: 25px;">
        <label class="layui-form-label"><span style="color: #f00;">*</span>省份：</label>
        <div class="layui-input-block">
            <select name="provinceid" lay-verify="required">
                <c:forEach var="p" items="${provinceList}">
                    <option value="${p.id}" ${p.id==city.provinceid ? 'selected': ''}>${p.provincename}</option>
                </c:forEach>
            </select>
        </div>
    </div>
    <div class="layui-form-item" style="margin-top: 25px;">
        ...
        <div class="layui-input-block">
            <input type="text" name="cityname" placeholder="请输入城市名称" lay-verify="required" autocomplete="off"
        ...
        form.on('submit(addProvince)', function (data) {
            ...
            $.ajax({
                ...
                success: function (result) {
                    ...
                    if (result.status == 0) {
                        window.parent.reflashCityTable();
                    }
